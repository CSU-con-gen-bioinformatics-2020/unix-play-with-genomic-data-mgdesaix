...
```
I got 10 for the fastq/DPCh_plate1_A06_S6.R1.fq.gz part, but 13 if I also count the ls and space after it.
```
...
# Using the `[` `]` copy the R1 files in `fastq` that belong to sample names (the part immediately after `DPCh_plate1_`) starting with an `A` or `B` to `fq_AB`.  
...
```{bash}
# dangerous, but I can't use rmdir because google drive always produces the icon? file...checked it before with ls to make sure I wasn't destroying lots o stuff!
rm -r d*
```
...
```
So you don't destroy your life.
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
Each of the directories {bad, directory, name, with, spaces} and all of their contents.
```
...
```
They are the same for the two files.
```
...
```{bash}
for sam in ./sam/*.sam; do wc $sam; done
```
...
```{bash}
# Did I miss something here, these are not the same as specified above?
for file in ./fastq/*R1*; do filegzwc=`gzcat $file | wc`; filewc=`wc $file`; echo $filegzwc $filewc; done
# First 3 scolumn will be gzcat | wc output
# Second 3 columns will be wc of output
```
...
```{bash}
## Still not working...check mattdesaix v staff designations
chmod 774 ./silly/FILE-rwxrwxr--
chmod 770 ./silly/FILE-rwxr-----
